...
/* NOTE: we lack a GlobalConfig parameter as nothing is inherited from
 * global options and at the time we need to call init() GlobalConfig is not
 * easily available.  This is slightly different than the use of the
 * XXXOptions pattern in syslog-ng, which usually uses an init method that
 * gets passed a GlobalConfig instance.
 */
void
