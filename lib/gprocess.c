...
g_process_format_pidfile_name(gchar *buf, gsize buflen)
{
  ...
  if (pidfile == NULL)
    {
      g_snprintf(buf, buflen, "%s/%s.pid", process_opts.pidfile_dir ? process_opts.pidfile_dir : get_installation_path(PATH_PIDFILEDIR), process_opts.name);
      pidfile = buf;
    }
  else if (pidfile[0] != '/')
    {
      /* complete path to pidfile not specified, assume it is a relative path to pidfile_dir */
      g_snprintf(buf, buflen, "%s/%s", process_opts.pidfile_dir ? process_opts.pidfile_dir : get_installation_path(PATH_PIDFILEDIR), pidfile);
      pidfile = buf;
  ...
  if (process_opts.mode != G_PM_FOREGROUND)
    {
      ...
      if (!cwd)
        cwd = get_installation_path(PATH_PIDFILEDIR);
