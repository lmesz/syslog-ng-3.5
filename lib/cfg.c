...
cfg_run_parser(GlobalConfig *self, CfgLexer *lexer, CfgParser *parser, gpointer *result, gpointer arg)
{
  ...
  self->lexer = lexer;
  cfg_args_set(self->lexer->globals, "syslog-ng-root", get_installation_path(PATH_PREFIX));
  cfg_args_set(self->lexer->globals, "syslog-ng-data", get_installation_path(PATH_DATADIR));
  cfg_args_set(self->lexer->globals, "module-path", module_path);
  cfg_args_set(self->lexer->globals, "include-path", get_installation_path(PATH_SYSCONFDIR));
